...
class ImageOrVideoDataset(Dataset):
    def __init__(
        ...
        super().__init__()
        self.dataset_file = dataset_file if isinstance(dataset_file, list) else [dataset_file]
        ...
        for data_file in self.dataset_file:
            ...
            with open(data_file, "r", encoding="utf-8") as file:
                for line in file.readlines():
                    try:
                        line = line.strip('\n')
                        assert self.delimiter in line, f"Expected delimiter {self.delimiter} in line {line}"
                        items = line.split(self.delimiter)
                        input_path = items['input_path']
                        caption_path = items['caption_path']
                        ...
                        if os.path.exists(caption_path) and os.path.isfile(caption_path):
                            data_path.append(caption_path)
                        else:
                            raise ValueError(f"Caption path {caption_path} does not exist.")
    ...
    def _preprocess_image(self, path) -> torch.Tensor:
        # TODO(aryan): Support alpha channel in future by whitening background
        image = TTF.Image.open(path).convert("RGB")
    ...
    def _preprocess_video(self, path):
        ...
        sample_interval = random.randint(self.frame_stride_min, self.frame_stride_max) # 可以采样到最大和最小值 
        indices = [self.start_skip_frms_num + sample_interval * i for i in range(video_num_frames // sample_interval)]
    ...
    def __getitem__(self, index: int):
        while True:
            try:
                ...
                continue
        num_frames = inputs[0].shape[0]
        if self.channel_order == "CTHW": 
            inputs = [input_.permute(1, 0, 2, 3).contiguous() for input_ in inputs]  # [T, C, H, W] -> [C, T, H, W]
